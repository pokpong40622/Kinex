// Full-screen embedded Unity SUPERSTAR STAGE (เวทีซุปตาร์ — Just Dance-style
// rehab pose game, the flagship KINEX game).
//
...
// ("dancestar") once the player is ready (and again on every unity_ready,
...
class DanceStarScreen extends StatefulWidget {
  const DanceStarScreen({super.key});

  @override
  State<DanceStarScreen> createState() => _DanceStarScreenState();
}

class _DanceStarScreenState extends State<DanceStarScreen> {
  ...
  void _selectGame() => sendToUnity('SceneRouter', 'LoadGame', 'dancestar');
  ...
  void _onUnityMessage(String data) {
    ...
    } else if (data.contains('dancestar_result')) {
      debugPrint(data);
      if (mounted) {
