// Full-screen embedded Unity Ancient Temple Treasure Hunt (ล่าสมบัติวิหารโบราณ).
//
...
// ("templehunt") once the player is ready (and again on every unity_ready,
...
class TempleHuntScreen extends StatefulWidget {
  const TempleHuntScreen({super.key});

  @override
  State<TempleHuntScreen> createState() => _TempleHuntScreenState();
}

class _TempleHuntScreenState extends State<TempleHuntScreen> {
  ...
  void _selectGame() => sendToUnity('SceneRouter', 'LoadGame', 'templehunt');
  ...
  void _onUnityMessage(String data) {
    ...
    } else if (data.contains('templehunt_result')) {
      debugPrint(data);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    // because every chamber's exercise detection runs on the live MediaPipe feed.
    return Scaffold(
