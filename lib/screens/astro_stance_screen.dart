// Full-screen embedded Unity AstroStance Game (นักบินอวกาศ — 3-lane space game).
//
...
// ("astrostance") once the player is ready (and again on every unity_ready,
...
class AstroStanceScreen extends StatefulWidget {
  const AstroStanceScreen({super.key});

  @override
  State<AstroStanceScreen> createState() => _AstroStanceScreenState();
}

class _AstroStanceScreenState extends State<AstroStanceScreen> {
  ...
  void _selectGame() => sendToUnity('SceneRouter', 'LoadGame', 'astrostance');
  ...
  void _onUnityMessage(String data) {
    ...
    } else if (data.contains('astrostance_result')) {
      debugPrint(data);
      if (mounted) {
