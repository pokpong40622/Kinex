/*
 * Kinex LEG-R bridge — ESP32-C3 Super Mini (RIGHT leg board)
 * -----------------------------------------------------------
 * BLE link between the Kinex Flutter app and the ESP32, used by the EMG
 * installation / MVC-calibration step. This is the RIGHT-leg board — its
 * twin, firmware/kinex_leg_L/, is identical except it advertises the "-L"
 * suffix and defaults to leg='L'. Both replace the legacy single-board
 ...
 *   suffix so the app pairs reliably: "Kinex-<HEX>-R" (e.g. "Kinex-3F7A-R").
 *   The app matches on the "-r" suffix (see matchesLegRBoard in
 ...
 *                       "START" with no leg defaults to this board's own side (R).
...
// Filled in setup() from the chip's own BLE MAC: "Kinex-<HEX>-R", e.g.
// "Kinex-3F7A-R". The app's Leg-R slot matches on the "-r" suffix (see
// matchesLegRBoard in lib/ble/ble_service.dart).
char DEVICE_NAME[24];
...
char          measuringLeg  = 'R';     // this board's side — default when START omits it
...
void setup() {
  Serial.begin(115200);
  delay(200);
  Serial.println("\nKinex LEG-R bridge (ESP32-C3) starting…");
  ...
  snprintf(DEVICE_NAME, sizeof(DEVICE_NAME), "Kinex-%02X%02X-R", mac[4], mac[5]);
